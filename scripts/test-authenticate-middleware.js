...

{
  const imp = signImpersonationToken(
    ...
    { id: 'mgr-1', role: 'property_manager' }
  );
  const { req, nextCalled } = run(`Bearer ${imp}`);
  check(nextCalled, 'manager impersonation token calls next');
  check(req.user?.impersonatorRole === 'property_manager', 'manager preview carries impersonatorRole');
}
