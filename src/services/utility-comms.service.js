...
} = require('../use-cases/utilities/period-utils');
...
// Lazy-required inside autoNotifyEligibleDrafts: uc03 notifies then calls back into this module.
...
async function autoNotifyEligibleDrafts({ userId, role }) {
  ...
  // Circular with this module (post-notify staff/tenant alerts) — keep require local.
  ...
  for (const bill of drafts) {
    ...
    if (shouldHoldAutoNotifyForElectricAmountSource(bill)) {
      console.warn(
    ...
    if (shouldHoldAutoNotifyForCalendarPhantom(bill, openProvider)) {
      console.warn(
