-- Remove legacy super_admin dev account; reassign FKs to owner (josemontero2002@gmail.com).
DO $migration$
DECLARE
  v_admin UUID;
  v_owner UUID;
  ...
  v_col   TEXT;
BEGIN
  SELECT id INTO v_admin FROM users WHERE email = 'admin@propertymanager.local';
  IF v_admin IS NULL THEN
    RAISE NOTICE 'admin@propertymanager.local not found — skip';
    RETURN;
  END IF;

  SELECT id INTO v_owner FROM users WHERE email = 'josemontero2002@gmail.com' LIMIT 1;
  IF v_owner IS NULL THEN
    SELECT id INTO v_owner FROM users WHERE role = 'owner' ORDER BY created_at LIMIT 1;
  END IF;
  IF v_owner IS NULL THEN
    RAISE EXCEPTION 'Cannot remove admin: no owner user exists to reassign references';
  END IF;
  ...
  LOOP
    ...
    IF EXISTS (
      ...
      EXECUTE format(
        ...
        v_tbl, v_col, v_col
      ) USING v_owner, v_admin;
    END IF;
  END LOOP;

  DELETE FROM refresh_tokens WHERE user_id = v_admin;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'property_assignments') THEN
    DELETE FROM property_assignments WHERE user_id = v_admin;
  END IF;
  DELETE FROM notifications WHERE user_id = v_admin;

  DELETE FROM users WHERE id = v_admin;
END $migration$;
